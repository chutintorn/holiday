...
import { getPriceDetail } from './api/getPriceDetail'; // ✅ NEW: Import price detail API
import JourneyTable, { flattenFlights } from './Journey.jsx';
import { useNavigate } from 'react-router-dom'; // ✅ NEW: Add navigate for redirection
...
const translations = {
  en: {
    ...
    loadingPrice: 'Fetching price details...'
  },
  th: {
    from: 'ต้นทาง',
    to: 'ปลายทาง',
    ...
    loadingPrice: 'กำลังดึงข้อมูลราคา...'
  }
};
...
export default function SearchForm() {
  ...
  const [loadingPriceDetail, setLoadingPriceDetail] = useState(false); // ✅ NEW: track price loading

  const dropdownRef = useRef(null);
  const t = translations[lang];
  const navigate = useNavigate(); // ✅ NEW: for routing
  ...
  const getData = async () => {
    ...
    try {
      ...
      const tableRows = flattenFlights(response.data,response.securityToken);
      setRows(tableRows);
      setSelectedId(null);
  ...
  const handleSubmitFlight = async (row) => {
    setLoadingPriceDetail(true);
    try {
      const detail = await getPriceDetail(row);
      navigate('/view-price-detail', { state: { priceDetail: detail, lang } });
    } catch (err) {
      console.error('❌ Price Detail Error:', err);
      alert('Error fetching price detail');
    } finally {
  ...
  return (
    <div className="search-form">
      ...
      {loadingPriceDetail && <p>{t.loadingPrice}</p>}
